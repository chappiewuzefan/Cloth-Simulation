#include "./cloth_code.h"
#include <math.h>
#include <stdio.h>
#include <stdlib.h>

void initMatrix(int n, double UNUSED(mass), double UNUSED(fcon),
                int UNUSED(delta), double UNUSED(grav), double sep,
                double rball, double offset, double UNUSED(dt), double **x,
                double **y, double **z, double **cpx, double **cpy,
                double **cpz, double **fx, double **fy, double **fz,
                double **vx, double **vy, double **vz, double **oldfx,
                double **oldfy, double **oldfz)
{
    int i, nx, ny;

    // Free any existing
    free(*x);
    free(*y);
    free(*z);
    free(*cpx);
    free(*cpy);
    free(*cpz);

    // allocate arrays to hold locations of nodes
    *x = (double *)malloc(n * n * sizeof(double));
    *y = (double *)malloc(n * n * sizeof(double));
    *z = (double *)malloc(n * n * sizeof(double));
    // This is for opengl stuff
    *cpx = (double *)malloc(n * n * sizeof(double));
    *cpy = (double *)malloc(n * n * sizeof(double));
    *cpz = (double *)malloc(n * n * sizeof(double));

    // initialize coordinates of cloth
    for (nx = 0; nx < n; nx++)
    {
        #pragma omp simd
        for (ny = 0; ny < n; ny++)
        {
            (*x)[n * nx + ny] = nx * sep - (n - 1) * sep * 0.5 + offset;
            (*z)[n * nx + ny] = rball + 1;
            (*y)[n * nx + ny] = ny * sep - (n - 1) * sep * 0.5 + offset;
            (*cpx)[n * nx + ny] = 0;
            (*cpz)[n * nx + ny] = 1;
            (*cpy)[n * nx + ny] = 0;
        }
    }

    // Throw away existing arrays
    free(*fx);
    free(*fy);
    free(*fz);
    free(*vx);
    free(*vy);
    free(*vz);
    free(*oldfx);
    free(*oldfy);
    free(*oldfz);
    // Alloc new
    *fx = (double *)malloc(n * n * sizeof(double));
    *fy = (double *)malloc(n * n * sizeof(double));
    *fz = (double *)malloc(n * n * sizeof(double));
    *vx = (double *)malloc(n * n * sizeof(double));
    *vy = (double *)malloc(n * n * sizeof(double));
    *vz = (double *)malloc(n * n * sizeof(double));
    *oldfx = (double *)malloc(n * n * sizeof(double));
    *oldfy = (double *)malloc(n * n * sizeof(double));
    *oldfz = (double *)malloc(n * n * sizeof(double));
    #pragma omp simd
    for (i = 0; i < n * n; i++)
    {
        (*vx)[i] = 0.0;
        (*vy)[i] = 0.0;
        (*vz)[i] = 0.0;
        (*fx)[i] = 0.0;
        (*fy)[i] = 0.0;
        (*fz)[i] = 0.0;
    }
}

void loopcode(int n, double mass, double fcon, int delta, double grav,
              double sep, double rball, double xball, double yball,
              double zball, double dt, double *x, double *y, double *z,
              double *fx, double *fy, double *fz, double *vx, double *vy,
              double *vz, double *oldfx, double *oldfy, double *oldfz,
              double *pe, double *ke, double *te)
{
    int i, j;
    double xdiff, ydiff, zdiff, vmag, damp;

    // update position as per MD simulation
    for (i = 0; i < n; i++)
    {
        #pragma omp simd
        for (j = 0; j < n; j++)
        {
            x[j * n + i] += dt * (vx[j * n + i] + dt * fx[j * n + i] * 0.5 / mass);
            oldfx[j * n + i] = fx[j * n + i];
        }
    }
    for (i = 0; i < n; i++)
    {
        #pragma omp simd
        for (j = 0; j < n; j++)
        {
            y[j * n + i] += dt * (vy[j * n + i] + dt * fy[j * n + i] * 0.5 / mass);
            oldfy[j * n + i] = fy[j * n + i];
        }
    }
    for (i = 0; i < n; i++)
    {
        #pragma omp simd
        for (j = 0; j < n; j++)
        {
            z[j * n + i] += dt * (vz[j * n + i] + dt * fz[j * n + i] * 0.5 / mass);
            oldfz[j * n + i] = fz[j * n + i];
        }
    }

    //	apply constraints - push cloth outside of ball
    for (i = 0; i < n; i++)
    {
        #pragma omp simd
        for (j = 0; j < n; j++)
        {
            xdiff = x[j * n + i] - xball;
            ydiff = y[j * n + i] - yball;
            zdiff = z[j * n + i] - zball;
            vmag = sqrt(xdiff * xdiff + ydiff * ydiff + zdiff * zdiff);
            if (vmag < rball)
            {
                // move the node to the nearest point on the surface of the ball
                double fv_x = xdiff / vmag * rball;
                double fv_y = ydiff / vmag * rball;
                double fv_z = zdiff / vmag * rball;
                x[j * n + i] = xball + fv_x;
                y[j * n + i] = yball + fv_y;
                z[j * n + i] = zball + fv_z;
                // calculate the unit vector of fvector
                double fv_norm = sqrt(fv_x * fv_x + fv_y * fv_y + fv_z * fv_z);
                double unit_fv_x = fv_x / fv_norm;
                double unit_fv_y = fv_y / fv_norm;
                double unit_fv_z = fv_z / fv_norm;
                // calculate the dot operation on the node velocity and the fvector
                double dot_fv_vel = vx[j * n + i] * unit_fv_x + vy[j * n + i] * unit_fv_y + vz[j * n + i] * unit_fv_z;
                // calculate the project of node velocity on the fvector
                double vel_pro_x = dot_fv_vel * unit_fv_x;
                double vel_pro_y = dot_fv_vel * unit_fv_y;
                double vel_pro_z = dot_fv_vel * unit_fv_z;
                // update the node velocity with damping factor
                vx[j * n + i] = (vx[j * n + i] - vel_pro_x) * 0.1;
                vy[j * n + i] = (vy[j * n + i] - vel_pro_y) * 0.1;
                vz[j * n + i] = (vz[j * n + i] - vel_pro_z) * 0.1;
            }
        }
    }

    *pe = eval_pef(n, delta, mass, grav, sep, fcon, x, y, z, fx, fy, fz);

    // Add a damping factor to eventually set velocity to zero
    damp = 0.995;
    for (i = 0; i < n; i++)
    {
        #pragma omp simd
        for (j = 0; j < n; j++)
        {
            vx[j * n + i] = (vx[j * n + i] +
                             dt * (fx[j * n + i] + oldfx[j * n + i]) * 0.5 / mass) *
                            damp;
            vy[j * n + i] = (vy[j * n + i] +
                             dt * (fy[j * n + i] + oldfy[j * n + i]) * 0.5 / mass) *
                            damp;
            vz[j * n + i] = (vz[j * n + i] +
                             dt * (fz[j * n + i] + oldfz[j * n + i]) * 0.5 / mass) *
                            damp;
        }
    }
    *ke = 0.0;
    for (i = 0; i < n; i++)
    {
        for (j = 0; j < n; j++)
        {
            *ke += vx[j * n + i] * vx[j * n + i] + vy[j * n + i] * vy[j * n + i] +
                   vz[j * n + i] * vz[j * n + i];
        }
    }
    *ke = *ke / 2.0;
    *te = *pe + *ke;
}

double eval_pef(int n, int delta, double mass, double grav, double sep,
                double fcon, double *x, double *y, double *z, double *fx,
                double *fy, double *fz)
{
    double pe, rlen, xdiff, ydiff, zdiff, vmag;
    int nx, ny, dx, dy;

    pe = 0.0;
    // loop over particles
    int index_n_node;
    int index_nx_n;
    for (nx = 0; nx < n; nx++)
    {
        for (ny = 0; ny < n; ny++)
        {
            index_nx_n = nx * n;
            index_n_node = index_nx_n + ny;

            fx[index_n_node] = 0.0;
            fy[index_n_node] = 0.0;
            fz[index_n_node] = -mass * grav;

            // loop over displacements
            // dx = nx; MAX(ny - delta, 0) < dy < MIN(ny, n);
            #pragma omp simd reduction(+: pe, fx[index_n_node], fy[index_n_node], fz[index_n_node])
            for (dy = MAX(ny - delta, 0); dy < ny; dy++)
            {
                // compute reference distance
                rlen = (double)((ny - dy)) * sep;
                // compute actual distance
                xdiff = x[index_nx_n + dy] - x[index_n_node];
                ydiff = y[index_nx_n + dy] - y[index_n_node];
                zdiff = z[index_nx_n + dy] - z[index_n_node];
                vmag = sqrt(xdiff * xdiff + ydiff * ydiff + zdiff * zdiff);
                // potential energy and force
                double vmag_sub_rlen = vmag - rlen;
                double f_const = fcon * vmag_sub_rlen / vmag;
                pe += fcon * vmag_sub_rlen * vmag_sub_rlen;
                fx[index_n_node] += f_const * xdiff;
                fy[index_n_node] += f_const * ydiff;
                fz[index_n_node] += f_const * zdiff;
            }
            // dx = nx; ny < dy < MIN(ny + delta + 1, n)
            #pragma omp simd reduction(+: pe, fx[index_n_node], fy[index_n_node], fz[index_n_node])
            for (dy = ny + 1; dy < MIN(ny + delta + 1, n); dy++)
            {
                // compute reference distance
                rlen = (double)(abs(ny - dy)) * sep;
                // compute actual distance
                xdiff = x[index_nx_n + dy] - x[index_n_node];
                ydiff = y[index_nx_n + dy] - y[index_n_node];
                zdiff = z[index_nx_n + dy] - z[index_n_node];
                vmag = sqrt(xdiff * xdiff + ydiff * ydiff + zdiff * zdiff);
                // potential energy and force
                double vmag_sub_rlen = vmag - rlen;
                double f_const = fcon * vmag_sub_rlen / vmag;
                pe += fcon * vmag_sub_rlen * vmag_sub_rlen;
                fx[index_n_node] += f_const * xdiff;
                fy[index_n_node] += f_const * ydiff;
                fz[index_n_node] += f_const * zdiff;
            }
            // MAX(nx - delta, 0) <= dx < nx;
            for (dx = MAX(nx - delta, 0); dx < nx; dx++) {
                int index_dx_n = dx * n;
                int nx_rel_dx = (nx - dx) * (nx - dx);
                // MAX(ny - delta, 0) < dy < MIN(ny + delta + 1, n)
                #pragma omp simd reduction(+: pe, fx[index_n_node], fy[index_n_node], fz[index_n_node])
                for (dy = MAX(ny - delta, 0); dy < MIN(ny + delta + 1, n); dy++)
                {
                    // compute reference distance
                    rlen = sqrt((double)(nx_rel_dx + (ny - dy) * (ny - dy))) * sep;
                    // compute actual distance
                    int index_diff = index_dx_n + dy;
                    xdiff = x[index_diff] - x[index_n_node];
                    ydiff = y[index_diff] - y[index_n_node];
                    zdiff = z[index_diff] - z[index_n_node];
                    vmag = sqrt(xdiff * xdiff + ydiff * ydiff + zdiff * zdiff);
                    // potential energy and force
                    double vmag_sub_rlen = vmag - rlen;
                    double f_const = fcon * vmag_sub_rlen / vmag;
                    pe += fcon * vmag_sub_rlen * vmag_sub_rlen;
                    fx[index_n_node] += f_const * xdiff;
                    fy[index_n_node] += f_const * ydiff;
                    fz[index_n_node] += f_const * zdiff;
                }
            }
            // nx + 1 <= dx < MIN(nx + delta + 1, n);
            for (dx = nx + 1; dx < MIN(nx + delta + 1, n); dx++)
            {
                int index_dx_n = dx * n;
                int nx_rel_dx = (nx - dx) * (nx - dx);
                // MAX(ny - delta, 0) < dy < MIN(ny + delta + 1, n)
                #pragma omp simd reduction(+: pe, fx[index_n_node], fy[index_n_node], fz[index_n_node])
                for (dy = MAX(ny - delta, 0); dy < MIN(ny + delta + 1, n); dy++)
                {
                    // compute reference distance
                    rlen = sqrt((double)(nx_rel_dx + (ny - dy) * (ny - dy))) * sep;
                    // compute actual distance
                    int index_diff = index_dx_n + dy;
                    xdiff = x[index_diff] - x[index_n_node];
                    ydiff = y[index_diff] - y[index_n_node];
                    zdiff = z[index_diff] - z[index_n_node];
                    vmag = sqrt(xdiff * xdiff + ydiff * ydiff + zdiff * zdiff);
                    // potential energy and force
                    double vmag_sub_rlen = vmag - rlen;
                    double f_const = fcon * vmag_sub_rlen / vmag;
                    pe += fcon * vmag_sub_rlen * vmag_sub_rlen;
                    fx[index_n_node] += f_const * xdiff;
                    fy[index_n_node] += f_const * ydiff;
                    fz[index_n_node] += f_const * zdiff;
                }
            }
        }
    }
    return 0.5 * pe;
}
