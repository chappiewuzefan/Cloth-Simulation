...
while iter < maxit:
    ...
    for node in nodes:
        ...
        # dist.mag = sqrt(self._x**2+self._y**2+self._z**2)
        if dist.mag < myball.radius:
            # Split velocity into normal(set to zero) and tangential, only tangential velocity is non-zero
            fvector = dist / dist.mag * myball.radius
            # velocity_projection = node.velocity.proj(fvector)
            velocity_projection = vector.dot(node.velocity, vector.norm(fvector)) * vector.norm(fvector)
            velocity_rejection = node.velocity - velocity_projection
            node.velocity = velocity_rejection
